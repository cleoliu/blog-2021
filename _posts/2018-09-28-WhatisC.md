...
- CC# 既簡單、強大、型別安全且為物件導向，同時保留 C 樣式語言的易讀性與簡潔性。 熟悉 C 和類似語言的任何人在適應 C# 方面很少有問題。 
...
> 抽像類別和介面這兩個東西就像是一個 template 模版，為了方便管理這些發展出用於不同情境應用的程式碼。
...
- Lambda 運算式 x => x * x 會指定名為 x 的參數，並傳回 x的平方值
