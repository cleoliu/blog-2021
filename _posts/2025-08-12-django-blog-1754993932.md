...
<body>
    ...
    <a href="{% url 'blog:post_list' %}" class="back-link">返回文章列表</a>
</body>
</html>
```
...
<body>
    ...
    <div class="comments-section">
        ...
        {% else %}
            <h4>發表評論</h4>
            <form action="{% url 'blog:post_detail' pk=post.pk %}" method="post" class="comment-form">
...
4.  **`Reverse for 'post_detail' with arguments '('',)' not found. 1 pattern tried: 'blog/(?P<pk>[0-9]+)/'`**
    ...
    *   **解法**: 檢查 `{% url 'blog:post_detail' pk=post.pk %}`，確保 `pk` 值存在且傳遞正確。
